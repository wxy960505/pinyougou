...
app.controller("brandcheckController",function($scope,$controller,$http, brandcheckService){
	...
	$scope.findByPage = function(page,rows){
		// 向后台发送请求获取数据:
        brandcheckService.findByPage(page,rows).success(function(response){
	...
	$scope.save = function(){
		...
		if($scope.entity.id != null){
			...
			// 保存
			object = brandcheckService.save($scope.entity);
		}
	...
	$scope.findById = function(id){
        brandcheckService.findById(id).success(function(response){
	...
	$scope.search = function(page,rows){
		// 向后台发送请求获取数据:
        brandcheckService.search(page,rows,$scope.searchEntity).success(function(response){
			$scope.paginationConf.totalItems = response.total;
			$scope.list = response.rows;
		});
	}
